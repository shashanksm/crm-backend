...
import io.shashanksm.crm.customer.account.repositories.PreferenceRepository;
...
public class SimpleAccountService implements AccountService {
	...
	public SimpleAccountService(AccountsRepository accountsRepository, ProfileRepository profileRepository,
			AddressRepository addressRepository, OrdersRepository ordersRepository,
			AccountProfileLinkRepository accountProfileLinkRepository, PreferenceRepository preferenceRepository) {
		super();
	...
	public Optional<AccountDto> findAccountById(Long accountId) throws AccountOperationException {
		...
		if (addresses != null) {
			List<AddressDto> addressDtos = addresses.stream().map(AddressDto::fromEntity).collect(Collectors.toList());
		...
		// preferences
		List<Preference> preferences = this.preferenceRepository.findByAccountId(accountId);
		if(preferences != null) {
			Map<String, String> prefMap = preferences.stream()
					.collect(Collectors.toMap(Preference::getKey, Preference::getValue));
			accountDto.setPreferences(prefMap);
		}
	...
	public Optional<AccountDto> createAccount(AccountDto accountDto) throws AccountOperationException {
		...
		// saving addresses
		List<Address> addresses = accountDto.getAdresses().stream().map(AddressDto::toEntity)
		...
		// saving orders
		List<Order> orders = accountDto.getOrders().stream().map(OrderDto::toEntity).collect(Collectors.toList());
		...
		//saving prefs
		List<Preference> preferences = accountDto.getPreferences().entrySet().stream()
				.map(entry->new Preference(entry.getKey(), entry.getValue()))
				.collect(Collectors.toList());
		for (Preference preference : preferences) {
			preference.setAccountId(account.getId());
		}
		this.preferenceRepository.saveAll(preferences);
